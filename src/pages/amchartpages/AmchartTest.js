    ...
    async function getallData(){
      const querySnapshot = await getDocs(collection(db, "studentTaskDetails"));
       ...
       let calculateByStudent = memberNamesFromBatch.map((member) => {
         ...
         return { name: member, steps:Number(((specifiedMemberOnloopFilter/((membersFilterFromBatch.length)*100))*100).toFixed(1)), pictureSettings: {
          src: require(`../Images/${`arvinth@citma`}.jpeg`).default
        }};
       });
...
debugger
  ...
  return (<>
    <div id="chartdiv"  style={{ width: "100%", height: "500px" , display: `${chartLoadChecker ? "block" : "none"}`}}></div>
     <div style={{display: `${!chartLoadChecker ? "block" : "none"}`}} className="spinner_chart"><Spin/></div>
  </>
  );
}
