...
function createData(name, overallAssignmentcalc, overallAttendance, overallAbsent, fromTrainer, pictureSettings) {
  return {
    name,
  ...
  return (
    <React.Fragment>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <Avatar style={{marginRight:"1rem"}} size={30} src= {require(`../Images/${row.pictureSettings}.jpeg`).default}/>
          {row.name} 
        </TableCell>
        <TableCell align="center"><Tag color="green">{row.batchNo}</Tag></TableCell>
              ...
              <Table size="small" aria-label="purchases">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="center">Day wise Self Assessment</TableCell>
                ...
                <TableBody>
                  {row.history.map((historyRow) => (
                    <TableRow key={historyRow.date}>
                      ...
                      <TableCell  align="center">{historyRow.daywiseselfassessment}</TableCell>
                      <TableCell align="center">{historyRow.amount}</TableCell>
                      <TableCell align="center">
                        {Math.round(historyRow.amount * row.fromTrainer * 100) / 100}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        async function getallData() {
          ...
          let calculateByStudent = memberNamesFromBatch.map((member) => {
            ...
            let specifiedMemberOnloopFilter = membersFilterFromBatch
              ...
              debugger
             let reduced = specifiedMemberOnloopFilter.reduce((acc, filteredForMember) => {
            ...
            return {
              ...
              history: specifiedMemberOnloopFilter.map(eachDataBymember=>{
                  return {date: eachDataBymember["Date-Pick"], daywiseselfassessment:eachDataBymember.slider+"", amount: 3}
              }),
